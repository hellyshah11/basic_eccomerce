...
@Component({
  ...
  template: `
   <div class="grid grid-cols-3 grid-cols-[3fr_1fr_1fr]">
    ...
    <app-qty-selector [quantity]="item().quantity" (qtyUpdated)="store.setItemsQuantity({productId: item().product.id, quantity: $event})"></app-qty-selector>
    ...
    <div class="flex flex-col items-end">
      ...
      <div class="flex -me-3">
        ...
        <button matIconButton style="color:#ba1a1a" (click)="store.removeFromCart(item().product)"> <mat-icon>delete</mat-icon></button>
      </div>
    </div>
   </div>
  `,
