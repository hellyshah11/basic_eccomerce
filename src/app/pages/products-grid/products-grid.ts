...
@Component({
  ...
  <mat-sidenav-container class="h-full">
    <mat-sidenav mode="side" [opened]="store.menuSideBar()" class="matSideNavbar" >
      <div class="p-6">
        <h2 class="text-lg text-gray-900">Categories</h2>

        <mat-nav-list>
          @for (list of categories();track list){
            <mat-list-item class="my-2" [activated]="list.toLowerCase() === category().toLowerCase() " [routerLink]="['/products',list]">
    ...
    <mat-sidenav-content class="bg-gray-100 p-6 h-full">
      ...
      @if (store.searchQuery()!=='') {
        <p class="text-base text-gray-600 mb-6">{{ store.filteredProductsForSearch().length}} products found for {{ store.searchQuery() }}</p>

        @if(store.filteredProductsForSearch().length === 0){
          <div class="flex flex-col items-center justify-center py-24 text-center bg-gray-100 rounded-xl h-100">
         
        </div>
        }@else{   
        <div class="responsive-grid">
          @for (product of store.filteredProductsForSearch(); track product.id) {
...
.mat-sidenav.mat-drawer.mat-sidenav.matSideNavbar.mat-drawer-side.mat-drawer-opened {
    width: 250px;
}
