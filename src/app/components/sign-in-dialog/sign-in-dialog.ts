...
import { SignInParams } from '../../layout/models/user';
import { SignUpDialog } from '../sign-up-dialog/sign-up-dialog';


@Component({
  selector: 'app-sign-in-dialog',
  imports: [ MatIcon, MatFormField, MatInput, MatSuffix, MatPrefix,MatDialogClose,MatIconButton,MatButton,ReactiveFormsModule],
  template: `
   <div class="p-8 max-w-[400px] flex flex-col">
    ...
    <form class="mt-6" [formGroup]="signInForm" (ngSubmit)="signIn()">
      ...
      <mat-form-field class="w-full mb-6">
       ...
       <button matSuffix mat-icon-button class="mr-2" (click)="passwordVisible.set(!passwordVisible())" type="button">
          <mat-icon [fontIcon]="passwordVisible() ? 'visibility_off':'visibility' "></mat-icon>
       </button>
      </mat-form-field>
      <button type="submit" matButton="filled" class="w-full" style="border-radius: 6px;">Sign In</button>
    </form>
...
export class SignInDialog {
  ...
  signInForm = this.fb.group({
    email: ['john@gamil.com',Validators.required],
